...
check_age_file <- function(path) {
    ...
    if (length(lines) >= 1 && grepl("^age-encryption\\.org/v1", lines[1], fixed = FALSE)) {
        return(TRUE)
    }
    ...
    return("File does not appear to be age-encrypted (no armor boundary or age header detected)")
}
